...
function Mod.PrintVisualZ(ent, tag, data, interval)
	...
	print(string.format(
		"[%s #%d] seq=%d:%s cycle=%.3f hullZ=%s renderZ=%s rDelta=%s active=%s groundZ=%s estZ=%s minZ=%s maxZ=%s Lloc=%s Lw=%s Lhit=%s Rloc=%s Rw=%s Rhit=%s footXY=%s foot3D=%s footDz=%s Lknee=%s Rknee=%s Lthigh=%s Rthigh=%s Lshin=%s Rshin=%s",
